...
wp.customize.controlConstructor[ 'xenial-tab' ] = wp.customize.Control.extend( {
	ready: function () {
			...
			if ( typeof $_GET[ 'autofocus[control]' ] !== 'undefined' && $_GET[ 'autofocus[control]' ] !== '' ) {
				jQuery( 'li[id^="customize-control-sidebars_widgets"]' ).on(
					'DOMNodeInserted', function () {
						jQuery( '.customizer-tab-label > label.' + $_GET[ 'autofocus[control]' ] ).trigger( 'click' );
					} );
			}
		} );
	...
	hideAllControls: function ( section ) {
		...
		// console.log( 'tab-control-id: ' + tabControl );
	...
	handleClick: function () {
		...
		jQuery( container ).find( '.customizer-tab-label' ).on( 'click', function () {
			jQuery( this ).parent().find( '.customizer-tab-label' ).removeClass( 'active' );
